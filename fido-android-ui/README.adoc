...
* **NFC antenna hints** overlay showing the device's physical NFC antenna locations (API 34+)
...
On supported devices (Android API 34+), the module can overlay animated indicators at the
device's physical NFC antenna locations to help users position their security key. The built-in
UI shows these automatically; you can also display them in your own composables.

Check support before showing the overlay, then place `NfcAntennaHint` on top of your layout:

[source,kotlin]
----
if (supportsNfcAntennaHint(context)) {
    NfcAntennaHint(
        iconSize = 64.dp,
...
`supportsNfcAntennaHint()` returns `false` on Android below API 34, when NFC is unavailable, or
when the device does not expose antenna location data — in which case `NfcAntennaHint` has no
visible effect.
...
* **Screen Capture Protection**: In non-debug builds the FIDO UI applies `FLAG_SECURE` (blocking screenshots and screen recording) and hides overlay windows while an operation is in progress.
