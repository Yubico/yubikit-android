 ...
 *
 * <p><a href="https://developers.yubico.com/OATH/YKOATH_Protocol.html">Protocol specification</a>.
...
public class OathSession extends ApplicationSession<OathSession> {
  ...
  public static final Feature<OathSession> FEATURE_TOUCH =
      new Feature.Versioned<>("Touch", 4, 2, 0);

  /** Support for credentials using the SHA-512 hash algorithm. */
  public static final Feature<OathSession> FEATURE_SHA512 =
      new Feature.Versioned<>("SHA-512", 4, 3, 1);

  /** Support for renaming a stored credential. */
  public static final Feature<OathSession> FEATURE_RENAME =
      new Feature.Versioned<>("Rename Credential", 5, 3, 0);
   ...
   */
  public OathSession(SmartCardConnection connection)
   ...
   *
   * <p>This ID is randomly generated upon invocation of {@link #reset()}.
   */
   ...
   *
   * <p>Once unlocked, the application will remain unlocked for the duration of the session. See the
   * YKOATH protocol specification for further details.
   *
  ...
  public boolean unlock(AccessKey validator) throws IOException, ApduException {
    ...
    try {
      ...
      byte[] data =
          protocol.sendAndReceive(new Apdu(0, INS_VALIDATE, 0, 0, Tlvs.encodeMap(request)));
      Map<Integer, byte[]> map = Tlvs.decodeMap(data);
      // return false if response from validation does not match verification
      boolean responsesEqual =
          MessageDigest.isEqual(
              validator.calculateResponse(clientChallenge), map.get(TAG_RESPONSE));
      if (responsesEqual) {
        challenge = null;
      }
  ...
  /**
   * Sets an Access Key derived from a password. Once a key is set, any usage of the credentials
   * stored will require the application to be unlocked via one of the validate methods. Also see
   * {@link #setAccessKey(byte[])}.
   *
  ...
  public void setAccessKey(byte[] key) throws IOException, ApduException {
    ...
    request.put(
        TAG_KEY,
        ByteBuffer.allocate(1 + key.length)
   ...
   *
   * <p>Credentials which use HOTP, or which require touch, will not be calculated. They will still
   * be present in the result, but with a null value. The current system time will be used for TOTP
   * calculation.
   *
   ...
   *
   * <p>Credentials which use HOTP, or which require touch, will not be calculated. They will still
   * be present in the result, but with a null value.
   *
   ...
   *
   * <p>Using this command a Credential can be used as an HMAC key to calculate a result for an
   ...
   */
  public byte[] calculateResponse(byte[] credentialId, byte[] challenge)
      throws IOException, ApduException, BadResponseException {
    Map<Integer, byte[]> request = new LinkedHashMap<>();
    request.put(TAG_NAME, credentialId);
    request.put(TAG_CHALLENGE, challenge);
    byte[] data =
        protocol.sendAndReceive(new Apdu(0, INS_CALCULATE, 0, 0, Tlvs.encodeMap(request)));
    byte[] response = Tlvs.unpackValue(TAG_RESPONSE, data);
    return Arrays.copyOfRange(response, 1, response.length);
  }

  /**
   * Returns a new Code for a stored Credential. The current system time will be used for TOTP
   * calculation.
   *
   ...
   * @throws ApduException in case of communication error
   */
  public Code calculateCode(Credential credential, @Nullable Long timestamp)
    ...
    byte[] data =
        protocol.sendAndReceive(new Apdu(0, INS_CALCULATE, 0, 1, Tlvs.encodeMap(requestTlv)));
   ...
   *
   * <p>The Credential ID (see {@link CredentialData#getId()}) must be unique to the YubiKey, or the
   * existing Credential with the same ID will be overwritten.
   *
   * <p>Setting requireTouch requires support for {@link #FEATURE_TOUCH}, available on YubiKey 4.2
   * or later. Using SHA-512 requires support for {@link #FEATURE_SHA512}, available on YubiKey
   * 4.3.1 or later.
   *
   ...
   * @throws ApduException in case of communication error
   */
  public Credential putCredential(CredentialData credentialData, boolean requireTouch)
      throws IOException, ApduException {
   ...
   *
   * <p>This functionality requires support for {@link #FEATURE_RENAME}, available on YubiKey 5.3 or
   * later.
   *
   ...
   * @throws ApduException in case of communication error
   */
  public void renameCredential(byte[] credentialId, byte[] newCredentialId)
      throws IOException, ApduException {
  ...
  public Credential renameCredential(
      Credential credential, String accountName, @Nullable String issuer)
      throws IOException, ApduException {
    if (!credential.deviceId.equals(deviceId)) {
      throw new IllegalArgumentException("The given credential belongs to a different device!");
    }
    byte[] newId =
        CredentialIdUtils.formatId(
            issuer, accountName, credential.getOathType(), credential.getPeriod());
    renameCredential(credential.getId(), newId);
    return new Credential(
        credential.deviceId, newId, credential.getOathType(), credential.isTouchRequired());
  }

  /**
   * Derives an access key from a password and the device-specific salt. The key is derived by
   * running 1000 rounds of PBKDF2 using the password and salt as inputs, with a 16 byte output.
   *
