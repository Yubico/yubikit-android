...
public class PcscSmartCardConnection implements SmartCardConnection {
    ...
    public byte[] sendAndReceive(byte[] apdu) throws IOException {
        try {
            Logger.trace(logger, "{} bytes sent over PCSC: {}", apdu.length, StringUtils.bytesToHex(apdu));
            if (apdu.length < 5) {
            ...
            Logger.trace(logger, "{} bytes received: {}", response.length, StringUtils.bytesToHex(response));
            return response;
        } catch (CardException e) {
            throw new IOException(e);
        }
