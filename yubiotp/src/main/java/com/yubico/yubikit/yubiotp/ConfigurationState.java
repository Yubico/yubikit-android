...
public class ConfigurationState {
   ...
   *
   * <p>This functionality requires support for {@link YubiOtpSession#FEATURE_CHECK_CONFIGURED},
   * available on YubiKey 2.1 or later.
   *
  ...
  public boolean isConfigured(Slot slot) {
    ...
    throw new UnsupportedOperationException(
        "Checking if a slot is configured is not supported on this YubiKey.");
  }

  /**
   * Checks if a configured slot is triggered by touch or not.
   *
   * <p>This functionality requires support for {@link
   * YubiOtpSession#FEATURE_CHECK_TOUCH_TRIGGERED}, available on YubiKey 3.0 or later. A slot is
   * triggered by touch if a user-initiated touch of the sensor causes it to output a payload over
   * the keyboard interface. Only HMAC-SHA1 challenge-response credentials are not triggered by
   * touch.
   *
   * @param slot the slot to check
  ...
  public boolean isTouchTriggered(Slot slot) {
    ...
    throw new UnsupportedOperationException(
        "Checking if a slot is triggered by touch is not supported on this YubiKey.");
  }
  ...
  public boolean isLedInverted() {
    return SlotConfiguration.EXTFLAG_LED_INV.isSupportedBy(version)
        && (flags & CONFIG_LED_INV) != 0;
  }
}
