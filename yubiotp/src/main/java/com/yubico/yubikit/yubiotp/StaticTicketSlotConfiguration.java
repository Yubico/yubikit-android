 ...
 *
 * <p>NOTE: {@link StaticPasswordSlotConfiguration} is a better choice in most cases!
 *
 * <p>A Static Ticket behaves like a Yubico OTP, but with all changing state removed.
 */
public class StaticTicketSlotConfiguration
    extends KeyboardSlotConfiguration<StaticTicketSlotConfiguration> {
  /**
   ...
   *
   * <p>NOTE: special=true implies digits=true, and cannot be used without it.
   *
   * @param upperCase if true the two first letters of the output string are upper-cased (default:
   *     false)
   * @param digit if true the first eight characters of the modhex alphabet are replaced with the
   *     numbers 0 to 7 (default: false)
   * @param special if true a ! is sent as the very first character, and digits is implied (default:
   *     false)
   * @return the configuration for chaining
   */
  public StaticTicketSlotConfiguration strongPassword(
      boolean upperCase, boolean digit, boolean special) {
   ...
   *
   * <p>NOTE: This feature is ONLY supported on YubiKey 2.x
   *
   * <p>Manual update is triggered by the user by holding the sensor pressed for 8-15 seconds. This
   * will generate a new random static ticket to be used, until manual update is again invoked.
   *
