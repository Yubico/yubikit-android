...
/**
 * Application to use and configure the OTP application of the YubiKey. This applications supports
 * configuration of the two YubiKey "OTP slots" which are typically activated by pressing the
 * capacitive sensor on the YubiKey for either a short or long press.
 *
 * <p>Each slot can be configured with one of the following types of credentials: - YubiOTP - a
 * Yubico OTP (One Time Password) credential. - OATH-HOTP - a counter based (HOTP) OATH OTP
 * credential (see <a
 * href="https://tools.ietf.org/html/rfc4226">https://tools.ietf.org/html/rfc4226</a>). - Static
 * Password - a static (non-changing) password. - Challenge-Response - a HMAC-SHA1 key which can be
 * accessed programmatically.
 *
 * <p>Additionally for NFC enabled YubiKeys, one slot can be configured to be output over NDEF as
 * part of a URL payload.
 */
public class YubiOtpSession extends ApplicationSession<YubiOtpSession> {
  ...
  public static final Feature<YubiOtpSession> FEATURE_CHECK_CONFIGURED =
      new Feature.Versioned<>("Check if a slot is configured", 2, 1, 0);

  /** Support for checking if a configured slot requires touch via the ConfigState. */
  public static final Feature<YubiOtpSession> FEATURE_CHECK_TOUCH_TRIGGERED =
      new Feature.Versioned<>("Check if a slot is triggered by touch", 3, 0, 0);

  /** Support for HMAC-SHA1 challenge response functionality. */
  public static final Feature<YubiOtpSession> FEATURE_CHALLENGE_RESPONSE =
      new Feature.Versioned<>("Challenge-Response", 2, 2, 0);

  /** Support for swapping slot configurations. */
  public static final Feature<YubiOtpSession> FEATURE_SWAP =
      new Feature.Versioned<>("Swap Slots", 2, 3, 0);

  /** Support for updating an already configured slot. */
  public static final Feature<YubiOtpSession> FEATURE_UPDATE =
      new Feature.Versioned<>("Update Slot", 2, 3, 0);
  ...
  public static void create(
      YubiKeyDevice device, Callback<Result<YubiOtpSession, Exception>> callback) {
    if (device.supportsConnection(OtpConnection.class)) {
      device.requestConnection(
          OtpConnection.class,
          value -> callback.invoke(Result.of(() -> new YubiOtpSession(value.getValue()))));
   ...
   *
   * <p>NOTE: Not all functionality is available over all transports. Over USB, some functionality
   * may be blocked when not using an OtpConnection.
   *
   ...
   */
  public YubiOtpSession(SmartCardConnection connection)
  ...
  public YubiOtpSession(SmartCardConnection connection, @Nullable ScpKeyParams scpKeyParams)
    ...
    backend =
        new Backend<SmartCardProtocol>(protocol, version, parseConfigState(version, statusBytes)) {
          ...
          {
            if (dummyStatus) { // We can't read the status, so use a dummy with both slots marked as
              // configured.
          ...
          @Override
          byte[] sendAndReceive(
              byte slot, byte[] data, int expectedResponseLength, @Nullable CommandState state)
  ...
  public YubiOtpSession(OtpConnection connection) throws IOException {
    ...
    backend =
        new Backend<OtpProtocol>(protocol, version, parseConfigState(version, statusBytes)) {
          @Override
          void writeToSlot(byte slot, byte[] data) throws IOException, CommandException {
            configurationState =
                parseConfigState(version, delegate.sendAndReceive(slot, data, null));
          }
  ...
  /**
   * Get the serial number of the YubiKey. Note that the EXTFLAG_SERIAL_API_VISIBLE flag must be set
  ...
  public int getSerialNumber() throws IOException, CommandException {
    return ByteBuffer.wrap(backend.sendAndReceive(CMD_DEVICE_SERIAL, new byte[0], 4, null))
        .getInt();
  }
   ...
   *
   * <p>NOTE: Attempting to delete an empty slot will under certain circumstances fail, resulting in
   * a {@link com.yubico.yubikit.core.otp.CommandRejectedException} being thrown. Prefer to check if
   * a slot is configured before calling delete.
   *
  ...
  public void putConfiguration(
    ...
    Logger.debug(
        ...
        slot);
    writeConfig(slot.map(CMD_CONFIG_1, CMD_CONFIG_2), configuration.getConfig(accCode), curAccCode);
  }

  /**
   * Update the configuration of a slot, keeping the credential.
   *
   * <p>This functionality requires support for {@link #FEATURE_UPDATE}, available on YubiKey 2.3 or
   * later.
   *
  ...
  public void updateConfiguration(
    ...
    if (!Arrays.equals(accCode, curAccCode)
        && getVersion().isAtLeast(4, 3, 2)
        && getVersion().isLessThan(4, 3, 6)) {
      throw new UnsupportedOperationException(
   ...
   *
   * <p>This functionality requires support for {@link #FEATURE_NDEF}, available on YubiKey 3 or
   * later.
   *
   ...
   *
   * <p>This functionality requires support for {@link #FEATURE_CHALLENGE_RESPONSE}, available on
   * YubiKey 2.2 or later.
   *
  ...
  private static ConfigurationState parseConfigState(Version version, byte[] status) {
    return new ConfigurationState(
        version, ByteBuffer.wrap(status, 4, 2).order(ByteOrder.LITTLE_ENDIAN).getShort());
  }
