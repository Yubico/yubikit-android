...
public class YubiOtpSession implements Closeable {
    ...
    private static final int SCAN_CODES_SIZE = com.yubico.yubikit.yubiotp.Config.FIXED_SIZE + com.yubico.yubikit.yubiotp.Config.UID_SIZE + com.yubico.yubikit.yubiotp.Config.KEY_SIZE;
    ...
    private com.yubico.yubikit.yubiotp.ConfigState configState;
     ...
     */
    public com.yubico.yubikit.yubiotp.ConfigState getStatus() {
        return configState;
    }
     ...
     * @throws CommandException in case of an error response from the YubiKey
     */
    public void deleteSlot(com.yubico.yubikit.yubiotp.Slot slot, @Nullable byte[] curAccCode) throws IOException, CommandException {
        writeConfig(
                slot.map(CMD_CONFIG_1, CMD_CONFIG_2),
                new byte[com.yubico.yubikit.yubiotp.Config.CONFIG_SIZE],
                curAccCode
        );
    }
     ...
     * @throws CommandException in case of an error response from the YubiKey
     */
    public void putConfiguration(com.yubico.yubikit.yubiotp.Slot slot, byte[] fixed, byte[] uid, byte[] key, byte extFlags, byte tktFlags, byte cfgFlags, @Nullable byte[] accCode, @Nullable byte[] curAccCode) throws IOException, CommandException {
        writeConfig(
                slot.map(CMD_CONFIG_1, CMD_CONFIG_2),
                com.yubico.yubikit.yubiotp.Config.buildConfig(fixed, uid, key, extFlags, tktFlags, cfgFlags, accCode),
                curAccCode
        );
    }
     ...
     * @throws CommandException in case of an error response from the YubiKey
     */
    public void updateConfiguration(com.yubico.yubikit.yubiotp.Slot slot, byte extFlags, byte tktFlags, byte cfgFlags, @Nullable byte[] accCode, @Nullable byte[] curAccCode) throws IOException, CommandException {
        writeConfig(
                slot.map(CMD_UPDATE_1, CMD_UPDATE_2),
                com.yubico.yubikit.yubiotp.Config.buildUpdateConfig(extFlags, tktFlags, cfgFlags, accCode),
                curAccCode
        );
    }
     ...
     * @throws CommandException in case of an error response from the YubiKey
     */
    public void setNdefConfiguration(com.yubico.yubikit.yubiotp.Slot slot, String uri, @Nullable byte[] curAccCode) throws IOException, CommandException {
        writeConfig(slot.map(CMD_NDEF_1, CMD_NDEF_2), com.yubico.yubikit.yubiotp.Config.buildNdefConfig(uri), curAccCode);
    }
     ...
     * @throws CommandException in case of an error response from the YubiKey
     */
    public byte[] calculateHmacSha1(com.yubico.yubikit.yubiotp.Slot slot, byte[] challenge, @Nullable CommandState state) throws IOException, CommandException {
     ...
     * ({@link #calculateHmacSha1(com.yubico.yubikit.yubiotp.Slot, byte[], CommandState)} how to use it after configuration)
     *
     ...
     * @throws CommandException in case of an error response from the YubiKey
     */
    public void putHmacSha1Key(com.yubico.yubikit.yubiotp.Slot slot, byte[] secret, boolean requireTouch) throws IOException, CommandException {
        if (version.isLessThan(2, 2, 0)) {
            throw new NotSupportedOperation("This operation is supported for version 2.2+");
        }
        ...
        byte[] key = new byte[com.yubico.yubikit.yubiotp.Config.KEY_SIZE];
        byte[] uid = new byte[com.yubico.yubikit.yubiotp.Config.UID_SIZE];
        ByteBuffer.wrap(ByteBuffer.allocate(com.yubico.yubikit.yubiotp.Config.KEY_SIZE + com.yubico.yubikit.yubiotp.Config.UID_SIZE).put(secret).array()).get(key).get(uid);

        byte cfgFlags = com.yubico.yubikit.yubiotp.Config.CFGFLAG_CHAL_HMAC | com.yubico.yubikit.yubiotp.Config.CFGFLAG_HMAC_LT64;
        if (requireTouch) {
            cfgFlags |= com.yubico.yubikit.yubiotp.Config.CFGFLAG_CHAL_BTN_TRIG;
        }
        ...
        putConfiguration(
                ...
                uid,
                key,
                (byte) (com.yubico.yubikit.yubiotp.Config.EXTFLAG_ALLOW_UPDATE | com.yubico.yubikit.yubiotp.Config.EXTFLAG_SERIAL_API_VISIBLE),
                com.yubico.yubikit.yubiotp.Config.TKTFLAG_CHAL_RESP,
                cfgFlags,
                null,
                null
        );
     ...
     * @throws CommandException in case of an error response from the YubiKey
     */
    public void putStaticPassword(com.yubico.yubikit.yubiotp.Slot slot, byte[] scanCodes) throws IOException, CommandException {
        if (version.isLessThan(2, 2, 0)) {
            throw new NotSupportedOperation("This operation is supported for version 2.2+");
        }
        ...
        byte[] fixed = new byte[com.yubico.yubikit.yubiotp.Config.FIXED_SIZE];
        byte[] uid = new byte[com.yubico.yubikit.yubiotp.Config.UID_SIZE];
        byte[] key = new byte[com.yubico.yubikit.yubiotp.Config.KEY_SIZE];
        ...
        putConfiguration(
                ...
                key,
                (byte) (com.yubico.yubikit.yubiotp.Config.EXTFLAG_ALLOW_UPDATE | com.yubico.yubikit.yubiotp.Config.EXTFLAG_SERIAL_API_VISIBLE),
                com.yubico.yubikit.yubiotp.Config.TKTFLAG_APPEND_CR,
                com.yubico.yubikit.yubiotp.Config.CFGFLAG_SHORT_TICKET,
                null,
                null
        );
    }
     ...
     * @throws CommandException in case of an error response from the YubiKey
     */
    public void putYubiOtpKey(com.yubico.yubikit.yubiotp.Slot slot, byte[] publicId, byte[] privateId, byte[] key) throws IOException, CommandException {
        putConfiguration(
                slot,
                publicId,
                privateId,
    ...
    public void putOathHotpKey(Slot slot, byte[] secret, boolean hotp8digits, int imf) throws IOException, CommandException {
        ...
        byte cfgFlags = 0;
        if (hotp8digits) {
            cfgFlags |= com.yubico.yubikit.yubiotp.Config.CFGFLAG_OATH_HOTP8;
        }
        ...
        putConfiguration(
                ...
                key,
                (byte) (com.yubico.yubikit.yubiotp.Config.EXTFLAG_ALLOW_UPDATE | com.yubico.yubikit.yubiotp.Config.EXTFLAG_SERIAL_API_VISIBLE),
                (byte) (com.yubico.yubikit.yubiotp.Config.TKTFLAG_OATH_HOTP | com.yubico.yubikit.yubiotp.Config.TKTFLAG_APPEND_CR),
                cfgFlags,
                null,
                null
        );
    ...
    private void writeConfig(byte commandSlot, byte[] config, @Nullable byte[] curAccCode) throws IOException, CommandException {
        configState = parseConfigState(version, backend.writeConfig(
                commandSlot,
                ByteBuffer.allocate(config.length + com.yubico.yubikit.yubiotp.Config.ACC_CODE_SIZE)
                        .put(config)
    ...
    private static com.yubico.yubikit.yubiotp.ConfigState parseConfigState(Version version, byte[] status) {
