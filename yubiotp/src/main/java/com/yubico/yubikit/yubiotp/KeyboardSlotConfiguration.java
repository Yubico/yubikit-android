...
abstract class KeyboardSlotConfiguration<T extends KeyboardSlotConfiguration<T>>
    extends BaseSlotConfiguration<T> {
  ...
  /**
   * Faster triggering when only slot 1 is configured. This option is always in effect on firmware
   * versions 3.0 and above.
   *
   * @param fastTrigger if true, trigger slot 1 quicker when slot 2 is not configured (default:
   *     true)
   * @return the configuration for chaining
   */
  ...
  /**
   * Adds a delay between each key press when sending output. This may sometimes be needed if the
   * host system isn't able to handle the default speed at which keystrokes are sent.
   *
   * <p>NOTE: These two flags can be combined to maximize the delay.
   *
  ...
  /**
   * Send scancodes for numeric keypad key presses when sending digits - helps with some keyboard
   * layouts.
   *
