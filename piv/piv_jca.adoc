...
`verificationPolicy`:: defines the verification policy for using the key. `VerificationPolicy.DEFAULT`, `VerificationPolicy.NEVER`, `VerificationPolicy.ONCE`, `VerificationPolicy.ALWAYS`, `VerificationPolicy.MATCH_ONCE` or `VerificationPolicy.MATCH_ALWAYS`
...
`pin`:: the PIV application PIN; valid PIN is required depending on the `VerificationPolicy` value and the operation which is planned to be performed with the resulting PrivateKey - signing and decrypting require PIN.

The `DEFAULT` values of `VerificationPolicy` and `TouchPolicy` depend on the `slot` value.
