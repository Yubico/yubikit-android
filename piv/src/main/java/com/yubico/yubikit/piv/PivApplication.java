...
public class PivApplication extends Iso7816Application {
    ...
    private static final byte TAG_AUTH_WITNESS = (byte)0x80;
    private static final byte TAG_AUTH_CHALLENGE = (byte)0x81;
    private static final byte TAG_AUTH_RESPONSE = (byte)0x82;
    private static final byte TAG_GEN_ALGORITHM = (byte)0x80;
    private static final byte TAG_OBJ_DATA = 0x53;
    private static final byte TAG_OBJ_ID = 0x5c;
    private static final byte TAG_CERTIFICATE = 0x70;
    private static final byte TAG_CERT_INFO = 0x71;
    private static final byte TAG_DYN_AUTH = 0x7c;
    private static final byte TAG_LRC = (byte) 0xfe;
    private static final byte TAG_PIN_POLICY = (byte) 0xaa;
    private static final byte TAG_TOUCH_POLICY = (byte) 0xab;
     ...
     * @param managementKey management key as hex byte array
    ...
    public void authenticate(byte[] managementKey) throws IOException, ApduException {
        ...
        byte[] witness = unwrapTlv(unwrapTlv(response, 0, TAG_DYN_AUTH), 0, TAG_AUTH_WITNESS);
        ...
        try {
            ...
            request = new Tlv(TAG_DYN_AUTH, TlvUtils.TlvToData(dataTlv)).getData();
            ...
            byte[] encryptedData = unwrapTlv(unwrapTlv(response, 0, TAG_DYN_AUTH), 0, TAG_AUTH_RESPONSE);
    ...
    public byte[] sign(Slot slot, Algorithm algorithm, byte[] message) throws IOException, ApduException {
        ...
        byte[] request = new Tlv(TAG_DYN_AUTH, TlvUtils.TlvToData(dataTlv)).getData();
    ...
    public void setManagementKey(byte[] managementKey) throws IOException, ApduException {
        ...
        stream.write(TDES);
        stream.write(new Tlv((byte)Slot.CARD_MANAGEMENT.value, managementKey).getData());
    ...
    public void putCertificate(Slot slot, X509Certificate certificate) throws IOException, ApduException {
        ...
        putObject(slot.object, TlvUtils.TlvToData(requestTlv));
    }
    ...
    public X509Certificate attest(Slot slot) throws IOException, ApduException {
        if (version.compare(new Version(4,3,0)) < 0) {
    ...
    public PublicKey generateKey(Slot slot, Algorithm algorithm, PinPolicy pinPolicy, TouchPolicy touchPolicy) throws IOException, ApduException {
        if (!SUPPORTED_ALGORITHMS.contains(algorithm)) {
            throw new IllegalArgumentException(String.format(Locale.ROOT, "Unsupported algorithm: 0x%02x", algorithm));
        }
        boolean isRsa = algorithm == Algorithm.RSA1024 || algorithm == Algorithm.RSA2048;

        if (isRsa && version.compare(new Version(4, 2, 0)) >= 0 && version.compare(new Version(4, 3, 5)) < 0) {
        ...
        if (touchPolicy == TouchPolicy.CACHED && version.compare(new Version(4, 3, 0)) < 0) {
        ...
        byte[] response = sendAndReceive(new Apdu(0, INS_GENERATE_ASYMMETRIC, 0, slot.value, new Tlv((byte)0xac, TlvUtils.TlvToData(tlvs)).getData()));

        // Tag '7F49' contains data objects for RSA or ECC
        SparseArray<byte[]> dataObjects = TlvUtils.parseTlvMap(unwrapTlv(response, 1, (byte) 0x49));
    ...
    public Algorithm importKey(Slot slot, PrivateKey key, PinPolicy pinPolicy, TouchPolicy touchPolicy) throws IOException, ApduException {
        ...
        sendAndReceive(new Apdu(0, INS_IMPORT_KEY, algorithm.value, slot.value, TlvUtils.TlvToData(tlvs)));
        return algorithm;
    }
    ...
    public byte[] getObject(byte[] objectId) throws IOException, ApduException {
        ...
        return unwrapTlv(responseData, 0, TAG_OBJ_DATA);
    }
    ...
    public void putObject(byte[] objectId, byte[] objectData) throws IOException, ApduException {
        ...
        sendAndReceive(new Apdu(0, INS_PUT_DATA, 0x3f, 0xff, TlvUtils.TlvToData(requestTlv)));
    }
    ...
    private int getRetriesFromCode(int statusCode) {
        ...
        if (version.compare(new Version(1, 0, 4)) < 0) {
     ...
     * @param offset offset to tlv within data byte array
     * @param tag tag that expected within provided tlv
     * @return internal Tlv
     * @throws UnexpectedTagException if tag is not found
     */
    private static byte[] unwrapTlv(byte[] data, int offset, byte tag) throws UnexpectedTagException {
        Tlv tlv = new Tlv(data, offset);
        int expectedTag = tag & 0xff;
