...
public class PivSession extends ApplicationSession<PivSession> {
  ...
  private static final int TAG_VERIFICATION_POLICY = 0xaa;
  ...
  private static final int INDEX_VERIFICATION_POLICY = 0;
   ...
   * for operations during this session and depending on slot {@link VerificationPolicy}.
   *
   * <p>Before calling this method, clients must verify that the authenticator is bio-capable and
   * not blocked for bio matching.
   *
  ...
  public SlotMetadata getSlotMetadata(Slot slot) throws IOException, ApduException {
    ...
    return new SlotMetadata(
        ...
        VerificationPolicy.fromValue(policy[INDEX_VERIFICATION_POLICY]),
   ...
   * generated on slot {@link #generateKey(Slot, KeyType, VerificationPolicy, TouchPolicy)}
   *
   * @param slot Key reference '9A', '9C', '9D', or '9E'. {@link Slot}.
   ...
   * @param verificationPolicy the PIN policy to check
  ...
  public void checkKeySupport(
    ...
    if (verificationPolicy != VerificationPolicy.DEFAULT || touchPolicy != TouchPolicy.DEFAULT) {
      require(FEATURE_USAGE_POLICY);
    ...
    if (version.isAtLeast(4, 4, 0) && version.isLessThan(4, 5, 0)) {
      ...
      if (verificationPolicy == VerificationPolicy.NEVER) {
        throw new UnsupportedOperationException(
            "VerificationPolicy.NEVER is not allowed on YubiKey FIPS");
      }
    }
  }
   ...
   * VerificationPolicy or TouchPolicy other than default require {@link #FEATURE_USAGE_POLICY},
   * available on YubiKey 4 or later. TouchPolicy.CACHED requires {@link #FEATURE_TOUCH_CACHED},
   * available on YubiKey 4.3 or later.
   *
   ...
   * @param verificationPolicy the PIN policy for using the private key
  ...
  public PublicKeyValues generateKeyValues(
      Slot slot, KeyType keyType, VerificationPolicy verificationPolicy, TouchPolicy touchPolicy)
      throws IOException, ApduException, BadResponseException {
    checkKeySupport(keyType, verificationPolicy, touchPolicy, true);

    Map<Integer, byte[]> tlvs = new LinkedHashMap<>();
    tlvs.put(TAG_GEN_ALGORITHM, new byte[] {keyType.value});
    if (verificationPolicy != VerificationPolicy.DEFAULT) {
      tlvs.put(TAG_VERIFICATION_POLICY, new byte[] {(byte) verificationPolicy.value});
    }
    ...
    Logger.debug(
        logger,
        "Generating key with verificationPolicy={}, touchPolicy={}",
        verificationPolicy,
        touchPolicy);
    byte[] response =
        protocol.sendAndReceive(
  ...
  public PublicKey generateKey(
      ...
      throws IOException, ApduException, BadResponseException {
    try {
      return generateKeyValues(slot, keyType, verificationPolicy, touchPolicy).toPublicKey();
   ...
   *
   * <p>KeyType P348 requires {@link #FEATURE_P384}, available on YubiKey 4 or later.
   * VerificationPolicy or TouchPolicy other than default require {@link #FEATURE_USAGE_POLICY},
  ...
  public KeyType putKey(
      Slot slot,
      PrivateKeyValues key,
      VerificationPolicy verificationPolicy,
      TouchPolicy touchPolicy)
      throws IOException, ApduException {
    KeyType keyType = KeyType.fromKeyParams(key);
    checkKeySupport(keyType, verificationPolicy, touchPolicy, false);
    ...
    Logger.debug(
        logger,
        "Importing key with verificationPolicy={}, touchPolicy={}",
        verificationPolicy,
        touchPolicy);
    protocol.sendAndReceive(
   ...
   * @throws IOException in case of connection error
   * @throws ApduException in case of an error response from the YubiKey
   * @deprecated use {@link #putKey(Slot, PrivateKeyValues, VerificationPolicy, TouchPolicy)}
   *     instead
   */
  @Deprecated
  public KeyType putKey(
      Slot slot, PrivateKey key, VerificationPolicy verificationPolicy, TouchPolicy touchPolicy)
      throws IOException, ApduException {
    return putKey(slot, PrivateKeyValues.fromPrivateKey(key), verificationPolicy, touchPolicy);
  }
