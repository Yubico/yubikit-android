...
class Padding {
  ...
  private static final Pattern ECDSA_HASH_PATTERN =
      Pattern.compile("^(.+)withECDSA$", Pattern.CASE_INSENSITIVE);
  private static final Pattern SHA_PATTERN =
      Pattern.compile("^SHA[0-9]+$", Pattern.CASE_INSENSITIVE);
   ...
   */
  static byte[] pad(KeyType keyType, byte[] message, Signature algorithm)
   ...
   */
  static byte[] unpad(byte[] decrypted, Cipher algorithm)
