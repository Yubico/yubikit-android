...
public class PinUvAuthProtocolV1Test {
    ...
    public void testEncodeCoordinate() {
        ...
        Assert.assertArrayEquals(
                ...
                PinUvAuthProtocolV1.encodeCoordinate(new BigInteger("115792089237316195423570985008687907853269984665640564039457584007913129639935"))
        );

        Assert.assertArrayEquals(
                TestUtils.decodeHex("7fffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"),
                PinUvAuthProtocolV1.encodeCoordinate(new BigInteger("57896044618658097711785492504343953926634992332820282019728792003956564819967"))
        );

        Assert.assertArrayEquals(
                TestUtils.decodeHex("01ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"),
                PinUvAuthProtocolV1.encodeCoordinate(new BigInteger("904625697166532776746648320380374280103671755200316906558262375061821325311"))
        );

        Assert.assertArrayEquals(
                TestUtils.decodeHex("00ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"),
                PinUvAuthProtocolV1.encodeCoordinate(new BigInteger("452312848583266388373324160190187140051835877600158453279131187530910662655"))
        );

        Assert.assertArrayEquals(
                TestUtils.decodeHex("007fffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"),
                PinUvAuthProtocolV1.encodeCoordinate(new BigInteger("226156424291633194186662080095093570025917938800079226639565593765455331327"))
        );
    }
}
