...
public class BasicWebAuthnClient implements Closeable {
  ...
  protected class MakeCredentialContext {
    ...
    protected Pair<Ctap2Session.CredentialData, ClientExtensionResults> makeCredential()
        throws IOException, CommandException, ClientError {
      final AuthParams authParamsResult =
          getAuthParams(pin, userVerification, permissions, rpId, state);
      ...
      for (Extension.RegistrationProcessor processor : registrationProcessors) {
        authenticatorInputs.putAll(processor.getInput(authParamsResult.pinToken));
      }
      ...
      if (residentKey || authParamsResult.internalUv) {
        if (residentKey) {
          if (!rkSupported) {
            throw new ClientError(
        ...
        if (authParamsResult.internalUv) {
      ...
      if (!(clientPin.getPinUvAuth() instanceof PinUvAuthDummyProtocol)
          && authParamsResult.pinToken != null) {
        pinUvAuthParam =
            clientPin.getPinUvAuth().authenticate(authParamsResult.pinToken, clientDataHash);
      ...
      for (Extension.RegistrationProcessor processor : registrationProcessors) {
        ...
        clientExtensionResults.add(
            processor.getOutput(attestationObject, authParamsResult.pinToken));
      }
  ...
  protected class GetAssertionContext {
    ...
    protected List<Pair<Ctap2Session.AssertionData, ClientExtensionResults>> getAssertions()
      ...
      final AuthParams getAuthResult =
          getAuthParams(pin, userVerification, permissions, rpId, state);
      ...
      PublicKeyCredentialDescriptor selectedCred =
          hasValidAllowList
              ? Utils.filterCreds(
                  ...
                  effectiveDomain,
                  clientPin.getPinUvAuth(),
                  getAuthResult.pinToken)
              : null;

      HashMap<String, Object> authenticatorInputs = new HashMap<>();
      for (Extension.AuthenticationProcessor processor : authenticationProcessors) {
        authenticatorInputs.putAll(processor.getInput(selectedCred, getAuthResult.pinToken));
      }

      Map<String, Boolean> ctapOptions =
          getAuthResult.internalUv
              ? Collections.singletonMap(OPTION_USER_VERIFICATION, true)
              : null;
      ...
      Integer pinUvAuthProtocolVersion = null;
      if (!(clientPin.getPinUvAuth() instanceof PinUvAuthDummyProtocol)
          && getAuthResult.pinToken != null) {
        pinUvAuthParam =
            clientPin.getPinUvAuth().authenticate(getAuthResult.pinToken, clientDataHash);
        pinUvAuthProtocolVersion = clientPin.getPinUvAuth().getVersion();
      }
      ...
      try {
        ...
        for (final Ctap2Session.AssertionData assertionData : assertions) {
          ClientExtensionResults clientExtensionResults = new ClientExtensionResults();
          for (Extension.AuthenticationProcessor processor : authenticationProcessors) {
            clientExtensionResults.add(processor.getOutput(assertionData, getAuthResult.pinToken));
          }
