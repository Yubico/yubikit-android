...
public class CredBlobExtensionTests {
    ...
    private void runTest(FidoTestState state) throws Throwable {
        ...
        state.withCtap2(session -> {
            Assume.assumeTrue("credBlob not supported",
            ...
            PublicKeyCredential cred = client
                    .makeCredential(
                            new CreationOptionsBuilder()
                                    .residentKey(true)
                                    .extensions(Collections.singletonMap(CRED_BLOB,
                                            Base64.toUrlSafeString(data)))
                                    .build()
                    );
            ...
            PublicKeyCredential cred = new ClientHelper(session)
                    .makeCredential(
                            new CreationOptionsBuilder()
                                    .residentKey(true)
                                    .extensions(Collections.singletonMap(CRED_BLOB,
                                            Base64.toUrlSafeString(CRED_BLOB_DATA)))
                                    .build()
                    );
    ...
    private byte[] getAssertionResult(PublicKeyCredential cred) {
        ...
        AuthenticatorData authenticatorData = AuthenticatorData.parseFrom(
                ByteBuffer.wrap(response.getAuthenticatorData()));
