...
It can list connected YubiKeys (via USB or NFC readers) and display device information, targeting a specific
device by serial number or fingerprint when multiple keys are connected.

== Commands

`list`:: List all connected YubiKeys with serial number, fingerprint, transport (USB/NFC), firmware version, and form factor.
