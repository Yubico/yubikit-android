...
===== NFC timeouts
NFC connection times out after `timeout` value of `NfcConfiguration()` object. Some operations which run on the YubiKey, for example generation of RSA keys, take long time. To avoid timeouts, start NFC discovery with configuration with reasonable timeout value.

[source,java]
----
    ...
    yubiKitManager.startNfcDiscovery(
        new NfcConfiguration().setTimeout(15000), // 15 seconds
    ...
----
