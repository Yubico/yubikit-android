 ...
 * }</pre>
 *
 * <p>The dialog can be customized by passing additional values in the intent.
 */
public class YubiKeyPromptActivity extends Activity {
  ...
  public static Intent createIntent(
      Context context, Class<? extends YubiKeyPromptAction> action, @StringRes int titleRes) {
  ...
  /**
   * Get a CommandState for use with some blocking YubiKey actions. The dialog will react to
   * KEEPALIVE_UPNEEDED, and the state will be cancelled if the user presses the cancel button.
   *
   ...
   *
   * <p>If {@link #provideResult(int, Intent)} has been called once this method returns, the
   ...
   *
   * <p>Internally this calls {@link #setResult(int, Intent)} with the given arguments, as well as
   * informing this Activity that it should finish once it is done handling any connected YubiKey.
   *
