...
public class FidoConnectionHandler extends InterfaceConnectionHandler<UsbFidoConnection> {
  ...
  // HID Usage Page item
  static final byte USAGE_PAGE_TAG = 0x06;
  // 0xF1D0 little-endian
  static final byte FIDO_USAGE_PAGE_LOW = (byte) 0xD0;
  static final byte FIDO_USAGE_PAGE_HIGH = (byte) 0xF1;
  ...
  static boolean hasFidoUsagePage(byte[] reportDescriptor) {
    if (reportDescriptor.length < 3) return false;
    for (int i = 0; i < reportDescriptor.length - 2; i++) {
      if (reportDescriptor[i] == USAGE_PAGE_TAG
          && reportDescriptor[i + 1] == FIDO_USAGE_PAGE_LOW
          && reportDescriptor[i + 2] == FIDO_USAGE_PAGE_HIGH) {
        return true;
      }
    }
    return false;
  ...
  private static int getReportDescriptorLength(UsbDeviceConnection connection, int interfaceId) {
