...
public class NfcConfiguration {
  ...
  /**
   * Setting this flag allows the caller to prevent the platform from playing sounds when it
   * discovers a tag.
   *
  ...
  /**
   * Setting this flag allows the caller to prevent the platform from performing an NDEF check on
   * the tags it
   *
  ...
  /**
   * Set it to true to shows view with settings nfc setting if NFC is disabled, otherwise start of
   * NFC session will return error in callback if no permissions/setting and allows user to handle
   * disabled NFC reader (show error or snackbar or refer to settings) Default value is false
   *
