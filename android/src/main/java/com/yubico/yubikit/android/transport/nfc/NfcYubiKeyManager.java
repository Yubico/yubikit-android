...
public class NfcYubiKeyManager {
  ...
  /**
   * Action for intent to tweak NFC settings in Android settings view on Q Android supports
   * Settings.Panel.ACTION_NFC, we might update with release on Q
   */
   ...
   */
  public NfcYubiKeyManager(Context context, @Nullable NfcDispatcher dispatcher)
  ...
  public void enable(
    ...
    if (checkAvailability(nfcConfiguration.isHandleUnavailableNfc())) {
      ...
      dispatcher.enable(
          ...
          nfcConfiguration,
          tag ->
              listener.invoke(new NfcYubiKeyDevice(tag, nfcConfiguration.getTimeout(), executor)));
   ...
   *
   * @param handleUnavailableNfc true if prompt user for turning on settings with UI dialog,
   *     otherwise returns error if no settings on or NFC_TRANSPORT is not available
