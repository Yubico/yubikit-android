...
NOTE: Version 3.1.0 adds the new experimental link:./fido-android-ui/[FIDO Android UI]
module, which provides a high-level FIDO2/WebAuthn client with built-in UI
components, WebView integration, and customizable theming.

NOTE: Version 3.0 breaks compatibility with 2.0. See
link:./doc/Migration_2.0-3.0.adoc[the migration guide] for some pointers on
migrating.
...
link:./fido/[Fido]::
This module adds FIDO2 support. Current implementation supports Webauthn Level 2 and CTAP 2.3 for managing FIDO credentials on YubiKeys.

link:./fido-android-ui/[FIDO Android UI] _(experimental)_::
This module provides a high-level FIDO2/WebAuthn client with built-in UI
components for Android applications. It handles the complete user interaction
flow including NFC/USB communication, PIN entry, and user prompts. It also
supports WebView integration for seamless passkey support in web applications.
...
link:./desktop/[Desktop] _(experimental)_::
This module provides concrete implementations of the core interfaces for desktop
Java applications, enabling YubiKey communication over USB and NFC (via external
readers such as OMNIKEY). It also supports multi-device selection when several
YubiKeys are connected.
...
The link:./desktop/[Desktop] module provides concrete implementations
of the core interfaces (USB and NFC connectivity) for building desktop Java
applications. See the module listing above for details.

Related modules:

* link:./testing-desktop/[testing-desktop]: SDK device test suite for desktop platforms
* link:./DesktopDemo/[DesktopDemo]: A sample application demonstrating basic usage

=== FAQ

==== Q1. Does YubiKit work with all versions of Android?
A1. All YubiKit modules should work on Android API 21+, except for the
link:./fido-android-ui/[FIDO Android UI] module which requires API 23+.
Yubico typically tests and supports n-1 per
