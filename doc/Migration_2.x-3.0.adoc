...
The SDK now uses JSpecify (`org.jspecify.annotations`) nullness annotations, enforcing stricter rules about which types can be null.
...
If you use FIDO/WebAuthn APIs, you must now provide client data through a `ClientDataProvider` implementation (not as a raw byte array).
...
== 5. FIDO Client and Error Handling Changes
