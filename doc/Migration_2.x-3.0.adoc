...
2. Nullness Annotations Migration
...
== 1. API Naming Changes
All abbreviation-based identifiers (e.g., PIN, RPID, URL) have been changed to CamelCase:
...
== 2. Nullness Annotations Migration
- Remove `javax.annotation` nullability annotations.
- Use JSpecify `org.jspecify.annotations.NullMarked` (at package level) and `@Nullable` (where needed).
...
- New class: `Ctap1Client` (implements `CtapClient`), supports older CTAP1 protocol for credential management and assertion flows.
...
- *Action*: Update custom build scripts to use the new structure. Refactor your `.gradle` files to rely on shared plugins from `build-logic`.
...
- Error class for authentication has been generalized and renamed.
...
- New fields added:
- `encCredStoreState` and `authenticatorConfigCommands` in `Ctap2Session.InfoData`
...
- *Action*: You may now write comprehensive tests for internal constants and session logic.
...
. **Update CamelCase API Naming:** Refactor all references and invocations of affected methods, classes, and constants.
. **Nullness Annotations:** Remove javax.annotation usages; add org.jspecify.annotations, annotate packages/classes as required.
. **FIDO Client Data:** Implement and use ClientDataProvider.
. **Replace Deprecated APIs:** Audit and refactor all references to now-removed APIs, check exception flows.
. **Refactor FIDO Architecture:** Swap BasicWebAuthnClient for Ctap2Client, review protocol branching logic.
. **Update Error Handling:** Use new CtapException error naming conventions and methods.
. **Migrate Build Scripts:** Refactor Gradle builds to use build-logic.
. **Authentication Flow Changes:** Ensure PIN and user verification errors are handled as per new class design.
. **Access CTAP2.3 Info:** Use new fields and methods in Ctap2Session.InfoData.
. **Enhance Test Coverage:** Utilize package-private session constructors and test internal logic; add Mockito as required.
. **USB Device Discovery:** Implement and configure VendorProductFilter/DeviceAccessFilter for YubiKey detection.

== 7. References

- Release notes: https://github.com/Yubico/yubikit-android/releases
- Key PRs: [#230](https://github.com/Yubico/yubikit-android/pull/230), [#231](https://github.com/Yubico/yubikit-android/pull/231), [#233](https://github.com/Yubico/yubikit-android/pull/233), [#237](https://github.com/Yubico/yubikit-android/pull/237), [#240](https://github.com/Yubico/yubikit-android/pull/240), [#238](https://github.com/Yubico/yubikit-android/pull/238), [#235](https://github.com/Yubico/yubikit-android/pull/235), [#234](https://github.com/Yubico/yubikit-android/pull/234), [#229](https://github.com/Yubico/yubikit-android/pull/229), [#243](https://github.com/Yubico/yubikit-android/pull/243), [#241](https://github.com/Yubico/yubikit-android/pull/241)
- Discussions for migration: https://github.com/Yubico/yubikit-android/discussions

For issues or migration questions, open an issue/discussion or consult Yubico support.

--
This document summarizes all core migration steps, API refactoring, and architectural upgrades for `yubikit-android` 3.0.