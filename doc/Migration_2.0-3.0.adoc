...
If you use FIDO/WebAuthn APIs, you must now provide client data through a `ClientDataProvider` implementation (not as a raw byte array). Use the provided `JsonClientDataProvider` class.
...
byte[] clientData = ...
ClientDataProvider clientDataProvider = new JsonClientDataProvider(clientData);
webAuthnClient.makeCredential(
