...
If you use FIDO/WebAuthn APIs, you must now provide client data through a `ClientDataProvider` implementation (not as a raw byte array). Use the provided `ClientDataProvider.fromClientDataJson` method.
...
byte[] clientData = ...
ClientDataProvider clientDataProvider = ClientDataProvider.fromClientDataJson(clientData);
webAuthnClient.makeCredential(
